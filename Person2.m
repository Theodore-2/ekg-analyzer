...
%combining the data
person2 = vertcat(normal, bradycardia, tachycardia);
plot(t,person2);

%tabulation of data with second values
veriTable = table(t',person2,'VariableNames',{'Time','Voltage'});

%saving the merged data as a txt file
disp(veriTable);

writetable(veriTable, 'person2.txt','Delimiter','\t');

%saving the merged data as a dat file
disp(veriTable);

writetable(veriTable, 'person2.dat','Delimiter','\t');